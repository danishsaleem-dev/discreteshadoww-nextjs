...
import { testimonials, googleReviewsUrl, type Testimonial } from "@/lib/testimonials";
import Reveal from "./Reveal";

function Stars({ n }: { n: number }) {
  return (
    <span className="text-sm tracking-[0.15em] text-gold-bright" aria-label={`${n} out of 5 stars`}>
      {"★★★★★".slice(0, n)}
    </span>
  );
}
      ...
      <div className="relative mx-auto mb-16 max-w-3xl px-6 text-center">
        ...
        <Reveal delay={0.1}>
          <div className="mt-7 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm text-paper-dim">
            <span className="font-display text-3xl text-paper">{avg.toFixed(1)}</span>
            <Stars n={5} />
            <a
            ...
            >
              from happy collectors on Google
            </a>
          </div>
        </Reveal>
      </div>
